...
'''
TODO:
some of them are categorization features
'''
class header_real_value_feature(feature_prototype):
    def scan(self,f_n, mini_len = 5):
        f = open(f_n,'rb')
        content = f.read()
        f.close()
        res =  extract_infos(content)
        for k in res.keys():
            self.counter[k] += 1
        if self.batch_normalization:
            self.update_feature_values(res)
        return res
